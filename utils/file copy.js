...
function Filter(path) {
  // 在require一个模块的时候（除了系统模块）， 都是从文件自己的相对路径去找。 但是当我们使用fs等模块时， 是根据cmd光标盘符去找。
  ...
  this.insert = function (obj, success, error) {
    ...
    // else return success("新增成功");
    // 在原文件内容中截取最后一个字符']' 再插入内容+']'
    // let length = JSON.stringify(obj).length;
    // fs.ftruncate(path, length - 1, function () {
    //   error("内容长度:" + length);
    //   if (err)   throw err;
  ...
  this.remove = function (obj, success, error) {
    ...
    let content = JSON.stringify(this.content, null, "\t");
